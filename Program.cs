...
public class Program
{
    ...
    public static async Task Main(string[] args)
    {
        ...
        UserInfo userInfo = new UserInfo{
                    ...
                    try{
                        ...
                        string jsonResponse = await PostRequestAsync("http://localhost:5000/Chat/all-users-chats", jsonRequest);
            ...
            switch(command){
                case "/chuuuat": 
                   ...
                   while (true){
                        ...
                        try{
                            ...
                            string jsonResponse = await PostRequestAsync("http://localhost:5000/Message/save", jsonRequest);   
                        }
                        catch (Exception ex){
                            Console.WriteLine("Ошибка: " + ex.Message);
                        }
                ...
                case "/new-chat":
                    ...
                    try{
                        ...
                        string jsonResponse = await PostRequestAsync("http://localhost:5000/Chat/new-chat", jsonRequest);
                        var options = new JsonSerializerOptions{
                            PropertyNameCaseInsensitive = true
                        };
                ...
                case "/chat-add":
                    ...
                    try{
                        ...
                        string jsonResponse = await PostRequestAsync("http://localhost:5000/Chat/add-user-in-chat", jsonRequest);
                        var options = new JsonSerializerOptions{
                            PropertyNameCaseInsensitive = true
                        };
                        ChatResponseData response = JsonSerializer.Deserialize<ChatResponseData>(jsonResponse, options);
    ...
    public static async Task GetLastMessagesInChat(string token, string chatID){
        ...
        try{
            ...
            string jsonResponse = await PostRequestAsync("http://localhost:5000/Message/get-chat-messages", jsonRequest);
            var options = new JsonSerializerOptions{
                PropertyNameCaseInsensitive = true
            };
    ...
    public static async Task GetNewMessages(string token, string chatID){
        while(true){
            ...
            try{
                ...
                string jsonResponse = await PostRequestAsync("http://localhost:5000/Message/get-new-chat-messages", jsonRequest);
                var options = new JsonSerializerOptions{
                    PropertyNameCaseInsensitive = true
                };
                ResponseMessageData response = JsonSerializer.Deserialize<ResponseMessageData>(jsonResponse, options);
